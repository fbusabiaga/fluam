...
bool runSchemeStokesLimitBigSystem(){
  ...
  cutilSafeCall(cudaMalloc((void**)&pVecinos,sizeof(vecinos)));
  initializeVecinosFull<<<numBlocks,threadsPerBlock>>>(vecino0GPU,
						       vecino1GPU,
						       vecino2GPU,
						       vecino3GPU,
						       vecino4GPU,
  ...
  while(step<numsteps){
    ...
    boundaryParticlesFunctionStokesLimitBigSystem(0,
						  numBlocksBoundary,
    ...
    kernelConstructWstokesLimitBigSystem<<<numBlocks,threadsPerBlock>>>(vxGPU, //Stored fx=S*Fx
									vyGPU,
									vzGPU,
									vxZ,
									vyZ,
    ...
    boundaryParticlesFunctionStokesLimitBigSystem(1,
						  numBlocksBoundary,
						  threadsPerBlockBoundary,
						  numBlocksNeighbors,
						  threadsPerBlockNeighbors,
    ...
    kernelConstructWstokesLimitBigSystem_2<<<numBlocks,threadsPerBlock>>>(vxGPU, //Stored fx=S*Fx+S*drift_p-S*drift_m
									  vyGPU,
									  vzGPU,
									  vxZ,
									  vyZ,
    ...
    boundaryParticlesFunctionStokesLimitBigSystem(2,
						  numBlocksBoundary,
						  threadsPerBlockBoundary,
						  numBlocksNeighbors,
						  threadsPerBlockNeighbors,
  ...
  cutilSafeCall(cudaFree(pVecinos));
